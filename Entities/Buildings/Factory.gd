...
func _tick_delivery_worker(world: World, citizen: Citizen, tick_minutes: int) -> void:
	match _delivery_phase:
		DELIVERY_PHASE_TO_STORAGE:
			...
			if citizen.has_method("stop_travel"):
				citizen.stop_travel()
			_delivery_quantity = _calculate_delivery_quantity(world, _delivery_target, _delivery_target_item, _delivery_source_commodity)
...
func _ensure_delivery_vehicle(world: World) -> bool:
	...
	if instance == null or instance is not Node3D or not instance.has_method("assign_delivery_driver"):
		push_warning("Factory: Could not instantiate delivery truck from %s." % TRUCK_SCENE_PATH)
		if instance != null:
			instance.queue_free()
		return false
...
func _has_vehicle_route_to(world: World, target: Building) -> bool:
	...
	if not world.has_method("get_vehicle_road_path"):
		return false
	var route: PackedVector3Array = world.get_vehicle_road_path(get_storage_point_global(), target.get_entrance_pos())
	return route.size() >= 2
