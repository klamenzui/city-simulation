...
@export_range(64, 2048, 1) var shore_mask_resolution: int = 1024
...
@export var shore_gradient_pixels: float = 155.0
@export var foam_band_pixels: float = 12.0
@export var shore_vertex_radius_pixels: int = 1
@export var include_name_keywords: PackedStringArray = PackedStringArray(["Beach", "Rock", "Island", "Ground", "Sand", "Flat"])
@export var exclude_name_keywords: PackedStringArray = PackedStringArray(["PalmTree", "Grass", "Flowers", "Fence", "Rope", "Torch", "Bones", "Skull", "Ship", "Boat", "Barrel", "Table", "Chair", "Seaweed", "Bridge", "Pier", "Dock"])
@export var sand_name_keywords: PackedStringArray = PackedStringArray(["Beach", "Sand"])
...
func generate_and_apply_shore_mask() -> void:
	...
	land.resize(total)
	var terrain_softness := PackedFloat32Array()
	terrain_softness.resize(total)

	var marked: int = _mark_land_from_node(root, land, terrain_softness, size)
	if marked <= 0:
		return
	...
	var edge_proximity := PackedFloat32Array()
	edge_proximity.resize(total)
	...
	var terrain_blur_radius: int = maxi(2, int(round(foam_band_pixels * 0.75)))
	var blurred_land := _blur_float_field(land_floats, size, terrain_blur_radius)
	var blurred_softness := _blur_float_field(terrain_softness, size, terrain_blur_radius)
	var shoreline_softness := PackedFloat32Array()
	shoreline_softness.resize(total)

	for i in range(total):
		var occ: float = blurred_land[i]
		if occ > 0.0001:
			shoreline_softness[i] = clampf(blurred_softness[i] / occ, 0.0, 1.0)
		else:
			shoreline_softness[i] = 0.5

	var foam_distance_limit: float = maxf(foam_band_pixels, 1.0)
	for i in range(total):
		if land[i] > 0:
			shore_values[i] = 0.0
			...
			shore_values[i] = clampf(1.0 - smoothstep(0.0, maxf(shore_gradient_pixels, 0.001), d), 0.0, 1.0)
			edge_proximity[i] = clampf(1.0 - d / foam_distance_limit, 0.0, 1.0)

	shore_values = _blur_float_field(shore_values, size, 2)
	edge_proximity = _blur_float_field(edge_proximity, size, 1)
	...
	for y in range(size):
		for x in range(size):
			...
			if land[idx] > 0:
				img.set_pixel(x, y, Color(0.0, 1.0, 1.0, shoreline_softness[idx]))
			else:
				img.set_pixel(x, y, Color(shore_values[idx], edge_proximity[idx], 0.0, shoreline_softness[idx]))
...
func _mark_land_from_node(node: Node, land: PackedByteArray, terrain_softness: PackedFloat32Array, size: int) -> int:
	var marked := 0
	if node is MeshInstance3D:
		marked += _mark_mesh_instance(node as MeshInstance3D, land, terrain_softness, size)
	for child in node.get_children():
		marked += _mark_land_from_node(child, land, terrain_softness, size)
	return marked

func _mark_mesh_instance(mi: MeshInstance3D, land: PackedByteArray, terrain_softness: PackedFloat32Array, size: int) -> int:
	if mi.mesh == null:
		return 0
	...
	var softness: float = _mesh_softness(mi)
	var marked := 0
	for surface_idx in range(mi.mesh.get_surface_count()):
		...
		if indices.size() >= 3:
			...
			while i + 2 < indices.size():
				marked += _mark_triangle(mi, vertices[indices[i]], vertices[indices[i + 1]], vertices[indices[i + 2]], land, terrain_softness, size, softness)
				i += 3
		else:
			var i := 0
			while i + 2 < vertices.size():
				marked += _mark_triangle(mi, vertices[i], vertices[i + 1], vertices[i + 2], land, terrain_softness, size, softness)
				i += 3
	return marked

func _mark_triangle(mi: MeshInstance3D, a: Vector3, b: Vector3, c: Vector3, land: PackedByteArray, terrain_softness: PackedFloat32Array, size: int, softness: float) -> int:
	...
	return _rasterize_triangle(av, bv, cv, land, terrain_softness, size, softness)
...
func _rasterize_triangle(a: Vector2, b: Vector2, c: Vector2, land: PackedByteArray, terrain_softness: PackedFloat32Array, size: int, softness: float) -> int:
	...
	if absf(area) < 0.001:
		return _mark_disk(a, land, terrain_softness, size, shore_vertex_radius_pixels, softness) + _mark_disk(b, land, terrain_softness, size, shore_vertex_radius_pixels, softness) + _mark_disk(c, land, terrain_softness, size, shore_vertex_radius_pixels, softness)
		...
		for x in range(min_x, max_x + 1):
			...
			if (w0 >= 0.0 and w1 >= 0.0 and w2 >= 0.0) or (w0 <= 0.0 and w1 <= 0.0 and w2 <= 0.0):
				...
				if land[idx] == 0:
					land[idx] = 1
					marked += 1
				terrain_softness[idx] = maxf(terrain_softness[idx], softness)
	return marked

func _edge(a: Vector2, b: Vector2, c: Vector2) -> float:
	return (c.x - a.x) * (b.y - a.y) - (c.y - a.y) * (b.x - a.x)

func _mark_disk(p: Vector2, land: PackedByteArray, terrain_softness: PackedFloat32Array, size: int, radius: int, softness: float) -> int:
...
func _mesh_softness(mi: MeshInstance3D) -> float:
	var text := _node_text(mi)
	...
	for keyword in rock_name_keywords:
		if text.find(String(keyword).to_lower()) >= 0:
			return 0.0
	if text.find("ground") >= 0 or text.find("island") >= 0 or text.find("flat") >= 0 or text.find("terrain") >= 0:
		return 0.65
	return 0.5

func _node_text(mi: MeshInstance3D) -> String:
