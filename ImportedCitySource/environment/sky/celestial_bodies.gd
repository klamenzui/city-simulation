...
func _ready():
	...
	# Load textures
	moon_texture = load("res://ImportedCitySource/environment/sky/Moon.png") if ResourceLoader.exists("res://ImportedCitySource/environment/sky/Moon.png") else null
	sun_texture = load("res://ImportedCitySource/environment/sky/Sun.png") if ResourceLoader.exists("res://ImportedCitySource/environment/sky/Sun.png") else null
	...
	for i in range(planets.size()):
		var texture_path = "res://ImportedCitySource/environment/sky/planet_" + str(i+1) + ".png"
...
func update_sun():
	...
	# Ð¡Ð¾Ð»Ð½Ñ†Ðµ Ð²Ð¸Ð´Ð½Ð¾ Ñ‚Ð¾Ð»ÑŒÐºÐ¾ Ð´Ð½ÐµÐ¼ (Ð¾Ñ‚ 6:00 Ð´Ð¾ 18:00)
	if time_manager:
		sun_time_visible = time_manager.current_time >= 6.0 and time_manager.current_time <= 18.0
	
	# ÐžÐ±Ð½Ð¾Ð²Ð»ÑÐµÐ¼ Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€Ñ‹ Ð² ÑˆÐµÐ¹Ð´ÐµÑ€Ðµ
	sky_material.set_shader_parameter("sun_color", sun_color)
	sky_material.set_shader_parameter("sun_size", sun_size)
...
func update_moon_phase():
	...
	# Map time to angle: moon_rise_hour_offset â†’ 0Â°, (moon_rise_hour_offset + 12) % 24 â†’ 180Â°, moon_set_hour_offset â†’ 360Â°
