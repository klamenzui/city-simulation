...
var _go_bench_cost: float = BalanceConfig.get_float("goap.energy.go_bench_cost", 1.0)
var _sleep_cost: float = BalanceConfig.get_float("goap.energy.sleep_cost", 0.6)
var _relax_bench_cost: float = BalanceConfig.get_float("goap.energy.relax_bench_cost", 0.85)
...
func _build_state(world, citizen) -> Dictionary:
	...
	state["at_bench"] = world.has_method("is_citizen_at_reserved_city_bench") and world.is_citizen_at_reserved_city_bench(citizen)
	state["has_home"] = citizen.home != null
	state["has_bench"] = world.has_method("has_available_city_bench_for") and world.has_available_city_bench_for(citizen, citizen.global_position)
...
func _build_actions() -> Array:
	...
	actions.append(GoapActionScript.new(
		"go_bench",
		_go_bench_cost,
		{"has_bench": true, "at_bench": false, "is_night": false},
		{"at_bench": true, "at_home": false}
	))
	actions.append(GoapActionScript.new(
		"sleep_home",
		_sleep_cost,
...
func _execute_first_action(action, world, citizen) -> bool:
	...
	match action.action_id:
		"go_home":
			...
			return true
		"go_bench":
			citizen.start_action(GoToBenchActionScript.new(), world)
			return true
		"sleep_home":
			citizen.start_action(SleepActionScript.new(), world)
			return true
		"relax_bench":
			citizen.start_action(RelaxAtBenchActionScript.new(), world)
			return true
		"relax_home":
