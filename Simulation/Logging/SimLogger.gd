...
static var _ai_log_file: FileAccess = null
...
static func start_new_session(mirror_to_stdout: bool = false) -> void:
	...
	_ai_log_file = _initialize_log_file(_ai_log_path, "AI Log", get_ai_log_path())
	if _log_file == null and _ai_log_file == null:
...
static func _get_open_file(path: String) -> FileAccess:
	if path == _log_path:
		return _log_file
	if path == _ai_log_path:
		return _ai_log_file
	return null

static func _close_open_files() -> void:
	if _log_file != null:
		_log_file.close()
		_log_file = null
	if _ai_log_file != null:
		_ai_log_file.close()
		_ai_log_file = null
